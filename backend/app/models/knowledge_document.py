import uuid
from datetime import datetime

from sqlalchemy import DateTime, String, Text, func
from sqlalchemy.orm import Mapped, mapped_column

from app.db.base import Base


class KnowledgeDocument(Base):
    __tablename__ = "knowledge_documents"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    title: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    source_type: Mapped[str] = mapped_column(String(50), nullable=False, default="manual")
    source_url: Mapped[str | None] = mapped_column(String(1000))
    source_file_path: Mapped[str | None] = mapped_column(String(1000))
    source_file_name: Mapped[str | None] = mapped_column(String(255))
    source_file_mime_type: Mapped[str | None] = mapped_column(String(255))
    source_file_size: Mapped[int | None] = mapped_column()
    source_file_storage: Mapped[str | None] = mapped_column(String(80))
    source_file_id: Mapped[str | None] = mapped_column(String(36), index=True)
    source_channel: Mapped[str | None] = mapped_column(String(50))
    source_external_user_id: Mapped[str | None] = mapped_column(String(255))
    parse_status: Mapped[str] = mapped_column(String(50), nullable=False, default="parsed")
    parse_error: Mapped[str | None] = mapped_column(Text)
    index_status: Mapped[str] = mapped_column(String(50), nullable=False, default="indexed")
    review_status: Mapped[str] = mapped_column(String(50), nullable=False, default="approved")
    publication_status: Mapped[str] = mapped_column(String(50), nullable=False, default="published")
    current_version: Mapped[int] = mapped_column(nullable=False, default=1)
    reviewed_by: Mapped[str | None] = mapped_column(String(36))
    reviewed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    customer_id: Mapped[str | None] = mapped_column(String(36), index=True)
    project_id: Mapped[str | None] = mapped_column(String(36), index=True)
    summary: Mapped[str | None] = mapped_column(Text)
    content_text: Mapped[str] = mapped_column(Text, nullable=False)
    created_by: Mapped[str] = mapped_column(String(36), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
